...
const MemberTaskPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const tasksResponse = await selectData('member_task', { member_id: userDetails.id });
    ...
    const handleAddTask = async () => {
        ...
        if (!newTask.task_name || !newTask.date) {
            setValidationErrors({
        ...
        try {
            ...
            setValidationErrors({ task_name: false, date: false }); // Reset validation errors

            // Refresh tasks
            const tasksResponse = await selectData('member_task', { member_id: selectedMember.id, month, year: Number(year) });
    ...
    const handleUpdateTask = async () => {
        ...
        // Validate required fields
        if (!currentTask.task_name || !currentTask.date) {
            setValidationErrors({
    ...
    const updateTaskStatus = async (task, newStatus, completionReason = null) => {
        ...
        try {
            ...
            await updateData('member_task', updatedTaskData, { id: task.id });
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <TextField
                                ...
                                size="small"
                                required
                                error={validationErrors.task_name}
                                helperText={validationErrors.task_name ? 'Task Name is required' : ''}
                            />
                        </Box>
