...
const CommunityPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Step 1: Fetch the current user's details
                const userDetails = await getUserDetails();
                setCurrentUser(userDetails);
        
                // Step 2: Fetch the team data for the current user
                const teamData = await selectData('members', { member_email: userDetails.email });
        
                // Extract unique team IDs from the team data
                const teamIds = [...new Set(teamData.data.map(member => member.team_id))];
        
                // Step 3: Fetch all members in the same team(s) using the team IDs
                const allMembers = [];
                for (const teamId of teamIds) {
                    const membersResponse = await selectData('members', { team_id: teamId });
                    allMembers.push(...membersResponse.data);
                }
        
                // Step 4: Filter out the current user from the team members
                const filteredMembers = allMembers.filter(member => member.member_email !== userDetails.email);
        
                // Step 5: Fetch user details for each team member from the users_accounts table
                const usersWithDetails = [];
                const uniqueEmails = new Set(); // To avoid duplicate user details
        
                for (const member of filteredMembers) {
                    if (!uniqueEmails.has(member.member_email)) {
                        const userResponse = await selectDataProfiles({ email: member.member_email });
                        if (userResponse.data.length > 0) {
                            usersWithDetails.push({
                                ...member,
                                ...userResponse.data[0], // Merge member and user details
                            });
                            uniqueEmails.add(member.member_email); // Mark this email as processed
                        }
                    }
                }
        
                // Step 6: Fetch team leaders for the teams the current user is part of
                const teamLeaders = [];
                for (const teamId of teamIds) {
                    const leadersResponse = await selectData('team_leaders', { team_id: teamId });
                    teamLeaders.push(...leadersResponse.data);
                }
        
                // Step 7: Fetch user details for each team leader from the users_accounts table
                for (const leader of teamLeaders) {
                    if (!uniqueEmails.has(leader.user_email)) {
                        const leaderResponse = await selectDataProfiles({ email: leader.user_email });
                        if (leaderResponse.data.length > 0) {
                            usersWithDetails.push({
                                ...leader,
                                ...leaderResponse.data[0], // Merge leader and user details
                                isLeader: true, // Add a flag to identify team leaders
                            });
                            uniqueEmails.add(leader.user_email); // Mark this email as processed
                        }
                    }
                }
        
                // Step 8: Set the users state with the combined list of team members and team leaders
