...
const TeamLeaderManagement = () => {
    ...
    const handleSearchChange = (e) => {
        ...
        const filtered = teamLeaders.filter(
            (leader) =>
                leader.full_name.toLowerCase().includes(query) || leader.email.toLowerCase().includes(query)
        );
    ...
    const handleAddTeamLeader = async () => {
        ...
        try {
            const createUserPayload = {
                ...
                password: '@CodeNet12345',
                role: newTeamLeaderForm.role,
            };
            await createNewUser(createUserPayload);
            ...
            <Box
                sx={{
                    ...
                    '&::-webkit-scrollbar-track': {
                        backgroundColor: theme.palette.mode === 'dark' ? '#333' : '#f1f1f1', // Scrollbar track color
                    },
                }}
            >
                {filteredTeamLeaders.slice(0, 6).map((leader) => ( // Display only 6 cards initially
                    <Card
                        key={leader.id}
                        sx={{
                            cursor: 'pointer',
