...
import {
  ...
  PersonOff, // Icon for inactive users
...
const AdminUserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const usersData1 = await selectDataProfiles({ company_id: userData.company_id });

        // Combine the data from usersData1 and usersData2
        const combinedData = [...usersData1.data];
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      handleSendPasswordResetEmail(selectedUser);
    } catch (error) {
  ...
  const handleSendEmail = (user) => {
    const subject = `${user.full_name} Account Credentials for SyncUp`;
    const body = `
Hello ${user.full_name},

Your password has been reset by the administration for security purposes. Please log in to your SyncUp account and create a new password.
...
CodeWorks & SyncUp Platform
  ...
  const handleSendPasswordResetEmail = (user) => {
    const subject = `🔐 Important: Password Reset for Your SyncUp Account`;
    const body = `
Hello ${user.full_name},
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <FormControl fullWidth margin="normal">
              ...
              <Select
                ...
                label="Role"
              >
                <MenuItem value="admin_com">Administration</MenuItem>
